...
export const useBoardData = (isLogged: boolean): UseBoardDataReturn => {
  ...
  const [currentUser, setCurrentUser] = React.useState<UserAPI | null>(null);
  const [folders, setFolders] = React.useState<FolderAPI[]>([]);
  const [posts, setPosts] = React.useState<PostAPI[]>([]);
  const [likes, setLikes] = React.useState<PostAPI[]>([]);
  ...
  const refreshFolders = async () => {
    ...
    setFolders(isOwnProfile ? data : data.filter((f) => !f.is_private));
  };
